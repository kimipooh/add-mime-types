...
function wamt_add_to_settings_menu(){
	...
	add_options_page(__('WP Add Mime Types Admin Settings', 'wp-add-mime-types'), __('Mime Type Settings','wp-add-mime-types'), $admin_permission, WAMT_PLUGIN_NAME, 'wamt_admin_settings_page');
}
...
function wamt_admin_settings_page(){
	...
	$wamt_form_submitted = isset( $_POST["wamt-form"] ) && sanitize_text_field( wp_unslash( $_POST["wamt-form"] ) );
	if( $wamt_form_submitted && current_user_can( $admin_permission ) ){
...
?>
<div class="add_mime_media_admin_setting_page_updated"><p><strong><?php esc_html_e('Updated', 'wp-add-mime-types'); ?></strong></p></div>

<div id="add_mime_media_admin_menu">
  <h2><?php esc_html_e('WP Add Mime Types Admin Settings', 'wp-add-mime-types'); ?></h2>
...
?>
     ...
     <fieldset style="border:1px solid #777777; width: 750px; padding-left: 6px; padding-bottom: 1em;">
		...
		<p><span style="color:red;"><?php  if(is_multisite() && is_plugin_active_for_network(WAMT_PLUGIN_BASENAME)) echo wp_kses_post( __( '* The site administrator cannot add the value for mime type because the multisite is enabled. <br/>Please contact the multisite administrator if you would like to add the value.', 'wp-add-mime-types' ) ); ?></span></p>

	<?php //  ?>
		<p>
			<input type="hidden" name="security_attempt_enable" value="no" />
			<input type="checkbox" name="security_attempt_enable" value="yes" <?php checked( isset($settings['security_attempt_enable']) && $settings['security_attempt_enable'] === "yes" ); ?> <?php disabled( !$permission || (is_multisite() && is_plugin_active_for_network(WAMT_PLUGIN_BASENAME)) ); ?>/> <?php esc_html_e('Enable the attempt to determine the real file type of a file by WordPress core.','wp-add-mime-types'); ?>
		</p>
		<p>
			<input type="hidden" name="filename_sanitized_enable" value="no" />
			<input type="checkbox" name="filename_sanitized_enable" value="yes" <?php checked( isset($settings['filename_sanitized_enable']) && $settings['filename_sanitized_enable'] === "yes" ); ?> <?php disabled( !$permission || (is_multisite() && is_plugin_active_for_network(WAMT_PLUGIN_BASENAME)) ); ?>/> <?php esc_html_e('Enable to sanitize the multiple file extensions within the filename by WordPress core.','wp-add-mime-types'); ?>
			</p>
		<p>
			<input type="hidden" name="file_type_debug" value="no" />
			<input type="checkbox" name="file_type_debug" value="yes" <?php checked( isset($settings['file_type_debug']) && $settings['file_type_debug'] === "yes" ); ?> <?php disabled( !$permission || (is_multisite() && is_plugin_active_for_network(WAMT_PLUGIN_BASENAME)) ); ?>/> <?php echo wp_kses_post( __( 'Record MIME type information detected by WordPress when a file is uploaded from the Media Library. When used together with "Enable the attempt to determine the real file type of a file by WordPress core.", the last detected extension, type, proper filename, and related MIME details can be reviewed on this settings screen. Disable this option after checking the information as needed.', 'wp-add-mime-types' ) ); ?>
				</p>
     </fieldset>
	 <br/>
<?php wamt_render_last_mime_debug_result(); ?>
	...
	<?php // If the permission is not allowed, the user can only read the setting. ?>
		<textarea name="mime_type_values" cols="100" rows="10" <?php disabled( !$permission || (is_multisite() && is_plugin_active_for_network(WAMT_PLUGIN_BASENAME)) ); ?>><?php if(isset($mimes) && is_array($mimes)) foreach ($mimes as $m_type=>$m_value) echo esc_textarea( $m_type . "\t= " . $m_value . "\n" ); ?></textarea>
     </fieldset>
...
?>
     <br/>
     <fieldset style="border:1px solid #777777; width: 750px; padding-left: 6px;">
		<legend><h3><?php esc_html_e('Past Add Values before Multisite function was enabled.','wp-add-mime-types'); ?></h3></legend>
		<p><span style="color:red;"><?php  echo wp_kses_post( __( '* This is for  multisite network administrators and site administrators.<br/> The following values are disabled after multisite function was enabled.', 'wp-add-mime-types' ) ); ?></span></p>
